...
### `keypair` is now required

As of the provider's latest commit, `keypair` (the named SSH keypair) is
required to create a baremetal server -- previously it was optional
alongside `keypair_id` and `public_key`. This module now marks `keypair`
`Required` accordingly. `keypair_id` and `public_key` remain optional and
can still be set alongside it.
...
`airtelcloud_baremetal` was introduced in provider `v1.2.4` (along with a
same-release follow-up patch to its network resolution and cloud-init
handling). There is no earlier provider release that has this resource, so
`versions.tf` pins `>= 1.2.4` rather than the lower floor used by other
service modules in this org.
...
| keypair | SSH keypair name. Forces replacement. | string | Yes | - |
